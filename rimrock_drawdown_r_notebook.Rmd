...
redds_sf_i %<>% 
  left_join(select(sp, year_lag, SWE_Apr), by = c("year" = "year_lag"), keep = F)
...
#build model
a <- glm(detrend_redds~ lag1_rim_af + SWE_Apr, data = redds_sf_i)
b <- glm(detrend_redds~  SWE_Apr, data = redds_sf_i)
c <- glm(detrend_redds~ lag1_rim_af, data = redds_sf_i)
d <- glm(detrend_redds~ 1, data = redds_sf_i)
...
#correlation
cor(select(redds_sf_i, detrend_redds, lag1_rim_af, SWE_Apr))

#model validation
DHARMa::simulateResiduals(a, plot = TRUE)
```

 Minimum pool and SWE are strongly correlated (r = 0.48). However, when both are used in a gaussian glm to model detrended redd counts from 1994 - 2023, the VIF is surprisingly low at 1.31. Model validation looks good. Backward stepwise selection using likelihood ratio tests, and a cutoff of p = 0.05 suggests that pool elevation (p = 0.02458), and not snowpack (p = 0.527), is a good predictor of redds the following year.

To be conservative, let's leave SWE in the model, and then plot model predictions


```{r, eval = FALSE}
#plot prediction from model
require(effects)
eff1 <- predictorEffect("lag1_rim_af", a)
...
Here's the model prediction, around 50 redds lost (in addition to the trend towards decline). The gray is 95% CI for predicted effect. Doesn't appraoch zero until the lowest pool observations.
...
- develop a new clean response variable (detrend using linear trend with year, not "detrend" function)
- choose an environmental variable that captures
  - PCA with snowpack on Apr 1, mean annual max temp, etc
  - look into the literature for "best"
- 
```{r}
sp <- read_csv("data/snowpack.csv")
sp <- sp[-1,c(1, 13)]
...
Still thinking about if this is an okay approach, but I pulled the Apr snow water equivalent (SWE) at the White Pass SNOTEL site into my R notebook. Minimum pool and SWE are strongly correlated (r = 0.48). When I use them both in a gaussian glm to model detrended redd counts from 1994 - 2023, the VIF is surprisingly low at 1.31. Model validation looks good. Backward stepwise selection using likelihood ratio tests, and a cutoff of p = 0.05 suggests that pool elevation (p = 0.02458), and not snowpack (p = 0.527), is a good predictor of redds the following year.

Here's the model prediction. The gray is 95% CI for predicted effect.

I don't know why pool elevation would show this relationship, but days at low pool would not. If I have time to QC it next week I will.

I fit a gaussian GLM on detrended redd counts from 1994 - 2023, using two expl variables (depth of low pool from the year before and April snow water equivalent at the White Pass SNOTEL site). 