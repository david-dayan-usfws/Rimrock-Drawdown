...
This notebook collects any computational work associated with understanding effects to bull trout from drawdowns of the Rimrock Resevoir. It supports s7 consultations, recovery projects and general questions about ecology, climate and water management.
...
Strogn linear trend towards decline is evident in the toime series of redd counts. We will need to detrend the data to identify impacts of opeations or environmental variables.
...
#build model
a <- glm(resid~ lag1_rim_af + SWE_Apr_lag1, data = redds_sf_i)
b <- glm(resid~  SWE_Apr_lag1, data = redds_sf_i)
c <- glm(resid~ lag1_rim_af, data = redds_sf_i)
d <- glm(resid~ 1, data = redds_sf_i)
...
a <- glm(resid~ lag1_rim_af + SWE_Apr, data = redds_sf_i)
b <- glm(resid~  SWE_Apr, data = redds_sf_i)
c <- glm(resid~ lag1_rim_af, data = redds_sf_i)
d <- glm(resid~ 1, data = redds_sf_i)
...
cor(select(redds_sf_i, resid, lag1_rim_af, SWE_Apr))
